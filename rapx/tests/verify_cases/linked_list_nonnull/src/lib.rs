...
impl<T: Copy> LinkedList<T> {
    // SOUND under T: Copy — ptr::read of a Copy type is safe.
    #[rapx::verify]
    ...
    // With T: Copy, returning &mut T from a pointer is still UB.
    // Use ptr::read to return by value instead, which is safe for Copy types.
    #[rapx::verify]
